...
python -c "from utils import merge_models_DeepRefine; merge_models('${input_dir}', '${models_prefix}', '${output_pdb}', '${total_num}', '8')"
...
for file in "$InputFolder2"/*; do
    if [ -f "$file" ]; then
        if [[ $file == *.pdb ]]; then
            ...
            phenix.molprobity $file quiet=False max_reasonable_bond_distance=50.0 probe_dots=False coot=False prefix="$filename"
            echo "$filename done"
            mv ./"${filename}.out" ${basicpath}/6-Molprobity/after_minimization/"${filename}.out"
        fi
    fi
done
echo "Molprobity: after Phenix minimization, all files executed."
python molprobity.py "${basicpath}/6-Molprobity" "after_minimization" "${pdb1}_${pdb2}_${type}_MDS_${frames}" ${total_num} 'Phenix'
...
echo "Molprobity: after Phenix minimization, all files executed."
python molprobity.py "${basicpath}/6-Molprobity" "after_minimization" "${pdb1}_${pdb2}_${type}_MDS_${frames}" ${total_num} 'DeepRefine'
